...
For each clip, either path does the same core steps:
  ...
  c. Embed the transcript with CLAP -> text_vector, used for text->audio
     semantic search.
  d. Score the clip against a fixed set of sound-event labels (see
     ingestion/events.py) and store the scores and resulting tags, so tag
     filters don't require a CLAP call at query time.
...
Run from the project root (so the embeddings package is importable):
  ...
  python -m ingestion.ingest url some-org/some-dataset --config en --transcript-column text
  ...
  python -m ingestion.ingest retag          # re-score stored clips, no re-embedding
"""
...
def _iter_points(client: QdrantClient, scroll_filter: Filter | None = None, page_size: int = 256):
...
def calibrate_events(source: str | None = None, qdrant_url: str = "http://localhost:6333") -> dict | None:
    """
    Computes per-label mean/std of event cosine scores from the audio vectors
    already stored in the collection, and saves them for z-score tagging.
    ...
    try:
        ...
        vecs, sources = [], {}
        for page in _iter_points(client, _source_filter(source)):
            for p in page:
...
def retag_clips(
    ...
    try:
        ...
        print(f"Retagging with {'z-score' if calib else 'probability'} rule.")
        ...
        for page in _iter_points(client, _source_filter(source)):
            ...
            for p in page:
                result = score_clip(
                    p.vector["audio"], names, label_matrix, logit_scale, calib, z_threshold, prob_threshold
                )
                ops.append(SetPayloadOperation(set_payload=SetPayload(payload=event_payload(result), points=[p.id])))
                old_tags = (p.payload or {}).get("tags", [])
                if old_tags != result["tags"]:
                    print(f"  {(p.payload or {}).get('transcript', '')[:50]!r}: {old_tags} -> {result['tags']}")
        ...
        for i, row in enumerate(ds):
            ...
            try:
                ...
                events = event_payload(
                    score_clip(audio_vec, label_names, label_matrix, logit_scale, calib, z_threshold, prob_threshold)
                )
                ...
                print(f"  [{i + 1}/{num_samples}] {transcript[:60]!r}  tags={events['tags']}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "source",
        choices=["dir", "url", "clear", "calibrate", "retag"],
        help=(
            "Where to ingest from; 'clear' to remove clips; 'calibrate' to compute event-score stats "
            "from stored clips; 'retag' to re-score stored clips without re-embedding."
        ),
    )
    parser.add_argument(
        "target",
        ...
        help=(
            ...
            "For 'calibrate' / 'retag': restrict to one dataset; omit it to use every clip."
        ),
    )
    ...
    parser.add_argument(
        "--z-threshold", type=float, default=Z_THRESHOLD, help="url/retag: z-score needed to tag (when calibrated)."
    )
    parser.add_argument(
        "--prob-threshold",
        type=float,
        default=PROB_THRESHOLD,
        help="url/retag: probability needed to tag the top event label (when not calibrated).",
    )
    ...
    if args.source == "clear":
        ...
        calibrate_events(args.target, qdrant_url=args.qdrant_url)
    elif args.source == "retag":
        n = retag_clips(args.target, args.qdrant_url, args.z_threshold, args.prob_threshold)
        print(f"Retagged {n} clips in '{COLLECTION}'.")
